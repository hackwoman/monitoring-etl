...
import {
  AppstoreOutlined, WarningOutlined, CheckCircleOutlined,
  FireOutlined, EyeOutlined,
...
const PageAnalysis: React.FC = () => {
  ...
  const handleEntityClick = (info: { guid: string; type_name: string; name: string }) => {
    setDrawerOpen(false);
    const path = { Page: '/page-analysis', Service: '/service-analysis', Host: '/host-analysis', NetworkDevice: '/network-analysis', HttpRequest: '/http-request-analysis' }[info.type_name];
    if (path) navigate(`${path}?name=${info.name}`);
  };

  const columns = [
    { title: '页面', dataIndex: 'name', sorter: (a: Entity, b: Entity) => a.name.localeCompare(b.name),
  ...
  return (
    <div>
      ...
      <h2 style={{ marginBottom: 16 }}><EyeOutlined style={{ marginRight: 8 }} />前端页面分析</h2>

      {stats && (
        <Row gutter={12} style={{ marginBottom: 16 }}>
          <Col span={4}><Card size="small"><Statistic title="总页面数" value={stats.total} /></Card></Col>
      ...
      {loading ? <Spin size="large" style={{ display: 'block', margin: '60px auto' }} /> :
        entities.length === 0 ? <Empty description="暂无 Page 实体" /> :
